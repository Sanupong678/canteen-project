...
export default {
  ...
  methods: {
    ...
    async loadRuemRimShops() {
      try {
        const response = await shopService.getRuemRimShops()
        if (response && Array.isArray(response)) {
          this.ruemrimShops = response
          console.log('โหลดข้อมูลร้านค้าสำเร็จ:', this.ruemrimShops)
        } else {
