...
export default {
  ...
  methods: {
    ...
    async loadDormityShops() {
      try {
        const response = await shopService.getDormityShops()
        if (response && Array.isArray(response)) {
          this.dormityShops = response
          console.log('โหลดข้อมูลร้านค้าสำเร็จ:', this.dormityShops)
        } else {
