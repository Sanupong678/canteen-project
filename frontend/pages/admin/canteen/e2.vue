...
export default {
  ...
  methods: {
    ...
    async loadE2Shops() {
      try {
        const response = await shopService.getE2Shops()
        if (response && Array.isArray(response)) {
          this.e2Shops = response
          console.log('โหลดข้อมูลร้านค้าสำเร็จ:', this.e2Shops)
        } else {
