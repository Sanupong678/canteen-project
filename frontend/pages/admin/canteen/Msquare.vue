...
export default {
  ...
  methods: {
    ...
    async loadmsquareShops() {
      try {
        const response = await shopService.getMsquareShops()
        if (response && Array.isArray(response)) {
          this.msquareShops = response
          console.log('โหลดข้อมูลร้านค้าสำเร็จ:', this.msquareShops)
        } else {
