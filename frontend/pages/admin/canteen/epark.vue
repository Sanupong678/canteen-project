...
export default {
  ...
  methods: {
    ...
    async loadEparkShops() {
      try {
        const response = await shopService.getEparkShops()
        if (response && Array.isArray(response)) {
          this.eparkShops = response
          console.log('โหลดข้อมูลร้านค้าสำเร็จ:', this.eparkShops)
        } else {
